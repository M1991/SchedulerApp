...
namespace SchedulerApp.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            CurrentUserTrapper cut = new CurrentUserTrapper();
            // This doesn't count login failures towards account lockout
            // To enable password failures to trigger account lockout, change to shouldLockout: true
        
            var sydd = db.Users.Where(m => m.Email.Equals(model.Email)).FirstOrDefault<ApplicationUser>();
            var sdd = UserManager.FindById(User.Identity.GetUserId());
            Session["UserFname"] = sydd.Email;
        
            var result = await SignInManager.PasswordSignInAsync(model.Email, model.Password, model.RememberMe, shouldLockout: false);
            switch (result)
            {
                case SignInStatus.Success:
                    Session["UserDept"] = sydd.Accesslvl;
                    Session["UserID"] = sydd.UserId;                   
                    cut.SomeFunction(model);                   
                    return LoginTrapper(returnUrl, model);
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, Accesslvl=model.Accesslvl, Fname=model.UserFname, Lname=model.UserLname, UserId=model.UserID, LastLogin=DateTime.Now};
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
                    //Added for registering a new roles
                    await this.UserManager.AddToRoleAsync(user.Id, model.Accesslvl);
                    ...
                    return RegistrationTrapper(model);
        ...
        [Authorize]
        public ActionResult LogoutTrapper()
        {
            UsersInfo dtChange = null;
           // var sttd="";
            var usr = System.Web.HttpContext.Current.User.Identity.GetEmailAdress();           
            using (var ctx = new ApplicationDbContext())
            {
                dtChange = ctx.DbUsersInfo.Where(s => s.UserEmail.Equals(usr)).FirstOrDefault<UsersInfo>();                
            }

            if (dtChange != null)
            {
                dtChange.LastLogout = DateTime.Now;
            }
